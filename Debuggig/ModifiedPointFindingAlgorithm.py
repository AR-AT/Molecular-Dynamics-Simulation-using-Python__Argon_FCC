...
def adaptive_equilibration(positions, velocities, forces, dt, box_length,
                           ...
                           max_threshold=1e-19, max_steps=10):
...
def find_melting_boiling_points(temperatures, potential_energies, window_size_ratio=0.1, std_multiplier=1.5):
    ...
    for i in range(1, len(significant_increases)):
        if significant_increases[i] and not significant_increases[i - 1]:
            if melting_point is None:
                melting_point = temperatures[i + window_size // 2]
            elif boiling_point is None:
                boiling_point = temperatures[i + window_size // 2]
                break
...
def main():
    ...
    sampling_step_range = [20]
        # 20000, 40000, 60000, 15000, 80000, 100000]
