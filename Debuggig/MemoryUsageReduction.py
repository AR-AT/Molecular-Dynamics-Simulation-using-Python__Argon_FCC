...
def adaptive_equilibration(positions, velocities, forces, dt, box_length,
                           verlet_algorithm, initial_equilibration_window=20,
                           initial_threshold=5e-21, min_threshold=1e-23,
                           max_threshold=5e-20, max_steps=20):
    ...
    with tqdm(total=equilibration_window, desc="Equilibration") as pbar:
        while equilibration_steps < max_steps:
            ...
            recent_potential_energies[index % window_size] = potential_energy
            index += 1
            ...
            if index >= window_size:
                ...
                if std_potential_energy > threshold:
                    ...
                    new_window_size = min(int(window_size * 1.3), len(recent_potential_energies) * 2)
...
def optimize_lattice_parameter():
    ...
    initial_guesses = np.linspace(4.5e-10, 6e-10, 1000)
    results = [minimize(potential_energy_for_lattice,
                        [guess], bounds=[(4.5e-10, 6e-10)]) for
...
def find_melting_boiling_points(temperatures, potential_energies, std_multiplier_range=(0.5, 5), window_size_ratio_range=(0.01, 5), step=0.01):
    ...
    for std_multiplier in np.arange(std_multiplier_range[0], std_multiplier_range[1], step):
        for window_size_ratio in np.arange(window_size_ratio_range[0], window_size_ratio_range[1], step):
            ...
            for i in range(1, len(significant_increases)):
                if significant_increases[i] and not significant_increases[i - 1]:
                    if melting_point is None:
                        melting_point_index = min(i + window_size // 2, len(temperatures) - 1)
                        if temperatures[melting_point_index] % 5 == 0:
                        ...
                        boiling_point_index = min(i + window_size // 2, len(temperatures) - 1)
                        if temperatures[boiling_point_index] % 5 == 0:
    ...
    if best_melting_point is None or best_boiling_point is None:
