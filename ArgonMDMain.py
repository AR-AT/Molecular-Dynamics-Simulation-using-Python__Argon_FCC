...
def velocity_verlet_adaptive(positions,
            velocities, forces, dt, box_length, max_force_threshold=1e-14):
...
def adaptive_equilibration(positions, velocities, forces, dt, box_length,
                           verlet_algorithm, initial_equilibration_window=200,
        ...
        while equilibration_steps < max_steps:
            ...
            if index >= window_size:
                ...
                if std_potential_energy > threshold:
                    ...
                    new_window_size = min(int(window_size * 1.005), len(recent_potential_energies) * 1.005)
...
def run_preliminary_steps_parallel(temperatures, seeds, verlet_algorithms, algorithm_names, time_steps, preliminary_steps):
    """Run preliminary steps in parallel."""
    args = [(T, seed, alg, dt, preliminary_steps) for alg in verlet_algorithms for T, seed, dt in zip(temperatures, seeds, time_steps)]
    with Pool(len(args)) as pool:
        results = pool.starmap(run_md_for_temperature, args)

    results_per_algorithm = [[] for _ in verlet_algorithms]

    for index, result in enumerate(results):
        if result is not None and all(r is not None for r in result):
            alg_index = index // len(temperatures)
            results_per_algorithm[alg_index].append(result)

    potential_energies_per_algorithm = []
    energy_drifts_per_algorithm = []
    best_time_steps = []

    for i, alg_results in enumerate(results_per_algorithm):
        if not alg_results:
            print(f"No results for algorithm {algorithm_names[i]}")
            logging.warning(f"No results for algorithm {algorithm_names[i]}")
            continue

        potential_energies = [res[1] for res in alg_results if res[1] is not None]
        energy_drifts = [res[2] for res in alg_results if res[2] is not None]

        if not potential_energies or not energy_drifts:
            print(f"No valid results for algorithm {algorithm_names[i]}")
            logging.warning(f"No valid results for algorithm {algorithm_names[i]}")
            continue

        try:
            logging.info(f"Potential energies for {algorithm_names[i]}: {potential_energies}")
            logging.info(f"Energy drifts for {algorithm_names[i]}: {energy_drifts}")
            best_time_step = time_steps[np.argmin(energy_drifts)]
        except IndexError:
            logging.error(f"IndexError when finding best time step for algorithm {algorithm_names[i]}")
            best_time_step = None
    ...
    best_time_steps = [ts for ts in best_time_steps if ts is not None]
...
def find_best_sampling_steps(T, seed, verlet_algorithm, time_step, sampling_step_range):
    ...
    for sampling_steps in sampling_step_range:
        try:
            ...
            logging.error(f"Error in sampling steps {sampling_steps}: {e}")
            continue
    return results
...
def main():
    ...
    preliminary_temperatures = np.arange(40, 205, 40)

    time_steps = [ 1e-15, 5e-16, 5e-15, 1e-16]
    ...
    if not best_time_steps:
        print("Error: No valid best time steps obtained.")
        logging.error("No valid best time steps obtained.")
        return
    ...
    sampling_step_range = [15000,20000, 30000,40000]

    # Collect results for all preliminary temperatures
    sampling_results = []
    for T, seed in zip(preliminary_temperatures, preliminary_seeds):
        results = find_best_sampling_steps(T, seed, best_verlet_algorithm, best_time_step, sampling_step_range)
        sampling_results.extend(results)

    # Calculate the average potential energy and energy drift for each sampling step
    average_results = {}
    for sampling_steps, potential_energy, energy_drift in sampling_results:
        if sampling_steps not in average_results:
            average_results[sampling_steps] = {'potential_energy': [], 'energy_drift': []}
        average_results[sampling_steps]['potential_energy'].append(potential_energy)
        average_results[sampling_steps]['energy_drift'].append(energy_drift)

    # Compute averages and normalize them
    max_ape = max([np.mean(average_results[sampling_steps]['potential_energy']) for sampling_steps in average_results], default=1)
    max_aed = max([np.mean(average_results[sampling_steps]['energy_drift']) for sampling_steps in average_results], default=1)

    # Calculate the combined score
    w_ape = 1
    w_aed = 2
    for sampling_steps in average_results:
        average_results[sampling_steps]['average_potential_energy'] = np.mean(average_results[sampling_steps]['potential_energy'])
        average_results[sampling_steps]['average_energy_drift'] = np.mean(average_results[sampling_steps]['energy_drift'])
        normalized_ape = average_results[sampling_steps]['average_potential_energy'] / (max_ape or 1)
        normalized_aed = average_results[sampling_steps]['average_energy_drift'] / (max_aed or 1)
        average_results[sampling_steps]['score'] = w_ape * normalized_ape + w_aed * normalized_aed

    # Choose the best sampling step based on the minimum score
    best_sampling_steps = min(average_results.items(), key=lambda x: x[1]['score'])[0]
