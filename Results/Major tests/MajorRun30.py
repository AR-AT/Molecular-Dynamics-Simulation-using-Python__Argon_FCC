...
def velocity_verlet_adaptive(positions,
            velocities, forces, dt, box_length, max_force_threshold=5e-15):
...
def adaptive_equilibration(positions, velocities, forces, dt, box_length,
                           verlet_algorithm, initial_equilibration_window=7000,
                           initial_threshold=5e-22, min_threshold=5e-25,
                           max_threshold=5e-20, max_steps=90000):
        ...
        while equilibration_steps < max_steps:
            ...
            if index >= window_size:
                ...
                if std_potential_energy > threshold:
                    threshold = min(threshold * 1.02, max_threshold)
                    ...
                    threshold = max(threshold * 0.98, min_threshold)
                    logging.info(f"((std_potential_energy < threshold)) threshold: {threshold:.5e}, MEAN: {mean_potential_energy:.5e}")
...
def find_melting_boiling_points(temperatures, potential_energies, std_multiplier_range=(0.0001, 0.005), window_size_ratio_range=(0.1, 0.25), step=0.005):
