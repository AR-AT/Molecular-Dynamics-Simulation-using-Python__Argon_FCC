...
def run_preliminary_steps_parallel(temperatures, seeds, verlet_algorithms, algorithm_names, time_steps, preliminary_steps):
    ...
    for i, alg_results in enumerate(results_per_algorithm):
        ...
        potential_energies = [res[1] for res in alg_results if res is not None]
        energy_drifts = [res[2] for res in alg_results if res is not None]
...
def main():
    ...
    time_steps = [1e-14, 1e-15, 5e-16, 1e-16, 5e-15]
    ...
    w_aed = 0.5
    for sampling_steps in average_results:
        normalized_ape = average_results[sampling_steps]['average_potential_energy'] / max_ape if max_ape != 0 else 0
        normalized_aed = average_results[sampling_steps]['average_energy_drift'] / max_aed if max_aed != 0 else 0
