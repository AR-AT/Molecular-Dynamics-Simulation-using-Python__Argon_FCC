...
def run_md_for_temperature(T, seed, verlet_algorithm, sampling_steps=20000):
    np.random.seed(seed)
    ...
    dt = 1e-15  # Time step for the simulation
...
# Generate PDF report
def generate_pdf_report(filename, melting_point, boiling_point, a_opt, temperatures, potential_energies, runtime, best_algorithm_name, reason):
    ...
    for i, line in enumerate(text_lines):
        c.drawString(50, height - 170 - i*20, line)
...
if __name__ == "__main__":
    ...
    # Run a short preliminary simulation to select the best Verlet algorithm
    ...
    for algorithm in verlet_algorithms:
        ...
        for T, seed in zip(temperatures[:3], seeds[:3]):
            potential_energy, energy_drift = run_md_for_temperature(T, seed, algorithm, sampling_steps=preliminary_steps)
    ...
    # Set sampling steps
    sampling_steps = 25000

    # Run the MD simulation in parallel using the best Verlet algorithm
    with Pool(6) as pool:
        results = pool.starmap(run_md_for_temperature, [(T, seed, best_verlet_algorithm, sampling_steps) for T, seed in zip(temperatures, seeds)])
    ...
    # Generate PDF report
    generate_pdf_report("MD_Simulation_Report.pdf", melting_point, boiling_point, a_opt, temperatures, potential_energies, runtime, best_algorithm_name, reason)
